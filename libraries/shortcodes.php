...
function cudazi_contact_sc($atts)
{
	...
	if ($to != "") {
		...
		<form class="standard" method="post" action="">
			...
			<div class="field clearfix">
				<input type="submit" style="font-weight: bold;" class="button" value="<?php echo $button; ?>"
					   name="cf_submit" id="cf_submit"/>
			</div>
		</form>
		<?php
	} else {
...
{
	extract(shortcode_atts(array(//no params at this time
	), $atts));
...
{
	extract(shortcode_atts(array(// none
	), $atts));
	return '</div>';
}
...
{
	extract(shortcode_atts(array(// no atts
	), $atts));
...
function cudazi_slider_sc($atts, $content = null)
{
	...
	if (have_posts()) {
		...
		if (!$slide_disable_nav) {
			$output .= "generatePagination: true, ";
		} else {
			$output .= "generatePagination: false, ";
		}
		if (!$slide_disable_nextprev) {
			$output .= "generateNextPrev: true, ";
		}
		if ($slide_delay) {
			$output .= "play: " . $slide_delay . ", ";
		}
		if ($slide_crossfade) {
