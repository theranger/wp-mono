...
function cudazi_thumb_linksto()
{
	...
	<p>
		...
		<em><?php _e('or', 'cudazi'); ?></em> <?php _e('Custom URL:', 'cudazi'); ?> <input type="text"
																						   style='width:300px; border-style:solid; border-width:1px;'
																						   name="featured_image_link_to_url"
																						   value="<?php echo $featured_image_link_to_url; ?>"/> <?php _e('(Full URL - Video, External Page, etc...)', 'cudazi'); ?>
	</p>
	<?php
}
...
function cudazi_post_settings_box()
{
	...
	<p><?php _e('Single post page layout:', 'cudazi'); ?> <select name="cudazi_post_settings">
			...
			foreach ($custom_post_settings as $custom_settings_key => $custom_settings_value) {
				if ($custom_settings_key == $cudazi_post_settings) {
					$sel = " selected='selected'";
				} else {
					$sel = "";
				}
	...
	<p><label><input type="checkbox" name="cudazi_post_hide_featured_single"
					 value="1" <?php if ($cudazi_post_hide_featured_single) {
				echo " checked='yes' ";
			} ?> /> <?php _e('Disable featured image on single post page', 'cudazi'); ?></label></p>
	<?php
}
...
function cudazi_portfolio_settings_box()
{
	...
	<p><?php _e('Attachment list displays all images in the media section. Attachment slider displays in a slider. Standard displays like a standard post.', 'cudazi'); ?>
		<br/><br/></p>
	<p>
		<strong><?php _e('Video / HTML Embed', 'cudazi'); ?></strong><br/><?php _e('Directly embed basic HTML, shortcodes or video embed code. Will replace the attached images on single portfolio pages.', 'cudazi'); ?>
	</p>
	<p><textarea style='font-family:monospace;width:75%;height:120px;border-style:solid; border-width:1px;' cols="50"
				 rows="5" name="portfolio_video_src_embed"><?php echo $portfolio_video_src_embed; ?></textarea></p>
	<?php
}
...
function cudazi_portfolio_template_box()
{
	...
	<p><label><?php _e('Posts Per Page', 'cudazi'); ?> <input type="text"
															  style='width:40px; border-style:solid; border-width:1px;'
															  name="portfolio_posts_per_page"
															  value="<?php echo $portfolio_posts_per_page; ?>"/></label>
	</p>
	<p><label><?php _e('Skills', 'cudazi'); ?> <input type="text"
													  style='width:300px; border-style:solid; border-width:1px;'
													  name="portfolio_skills" value="<?php echo $portfolio_skills; ?>"/></label>
		<?php
...
function cudazi_slider_settings_box()
{
	...
	<p><label><?php _e('Slide Width', 'cudazi'); ?></label> <input type="text"
																   style='width:70px; border-style:solid; border-width:1px;'
																   name="slide_width"
																   value="<?php echo $slide_width; ?>"/> <?php _e('(e.g. 400px)', 'cudazi'); ?>
	</p>
	<p><label><?php _e('Slide Height', 'cudazi'); ?></label> <input type="text"
																	style='width:70px; border-style:solid; border-width:1px;'
																	name="slide_height"
																	value="<?php echo $slide_height; ?>"/> <?php _e('(e.g. 300px)', 'cudazi'); ?>
	</p>
