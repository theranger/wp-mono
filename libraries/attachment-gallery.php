...
function cudazi_attachment_gallery($options)
{
	...
	if (have_posts()) {
		while (have_posts()) {
			...
			if ($slide_counter > 0 && $display == 'slider') {
			...
			if ($display != 'slider') {
				...
				if ($attachment_img_title || $attachment_img_description) {
					...
					if ($attachment_img_title) {
						$output .= "<h3>" . apply_filters('the_title', $attachment_img_title) . '</h3>';
					}
					if ($attachment_img_description) {
						$output .= "<p>" . apply_filters('the_title', $attachment_img_description) . '</p>';
					}
					$output .= "</div><!--//caption-->";
